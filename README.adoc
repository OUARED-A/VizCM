Graph dataabse Cost Model Application
==========================
For supporting the exploration of database cost model, we persist the graph model
describing in section 3.2 into a graph database DB-CM ; among many available graph
databases, we have chosen Neo4j (https://neo4j.com/), currently the leading open
source graph database, used by several companies. We map to Cypher (Neo4js query
language) exploration queries which are progressively built by our query interface
...
In our case, we use the repository from a `DB-CM` to compute the graph representation for the visualization.
